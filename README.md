...
HOST = "0.0.0.0" PORT = 5000 clientes = [] usuarios = {}
Explicacion:
...
Función broadcast()
def broadcast(mensaje, remitente): for cliente in clientes: if cliente != remitente: try: cliente.send(mensaje.encode('utf-8')) except: pass
Explicacion:
...
def manejar_cliente(cliente): try: cliente.send("Ingrese su nickname: ".encode('utf-8')) nickname = cliente.recv(1024).decode('utf-8') usuarios[cliente] = nickname
Explicacion:
...
print(f"{nickname} se ha conectado.") broadcast(f"{nickname} se ha unido al chat.", cliente)
Explicacion:
...
while True: mensaje = cliente.recv(1024).decode('utf-8')
Explicacion:
...
Comando /usuarios
if mensaje == "/usuarios": lista = ", ".join(usuarios.values()) cliente.send(f"Usuarios conectados: {lista}".encode('utf-8'))
Explicacion:
...
Comando /api
elif mensaje == "/api": respuesta = requests.get("https://catfact.ninja/fact").json() cliente.send(f"Dato curioso: {respuesta['fact']}".encode('utf-8'))
Explicacion:
...
Comando inexistente
else: cliente.send("Comando no reconocido.".encode('utf-8'))
Explicacion:
...
Mensajes normales
else: texto = f"{nickname}: {mensaje}" print(texto) broadcast(texto, cliente)
Explicacion:
...
finally: if cliente in clientes: clientes.remove(cliente) if cliente in usuarios: nombre = usuarios[cliente] del usuarios[cliente] broadcast(f"{nombre} ha abandonado el chat.", cliente) cliente.close()
Explicacion:
...
servidor = socket.socket(socket.AF_INET, socket.SOCK_STREAM) servidor.bind((HOST, PORT)) servidor.listen()
Explicacion:
...
while True: cliente, direccion = servidor.accept() clientes.append(cliente) hilo = threading.Thread(target=manejar_cliente, args=(cliente,)) hilo.start()
Explicacion:
...
cliente = socket.socket(socket.AF_INET, socket.SOCK_STREAM) cliente.connect((HOST, PORT))
Explicacion:
...
Función recibir()
def recibir(): while True: try: mensaje = cliente.recv(1024).decode('utf-8') print(mensaje)
Explicacion:
...
except: print("Desconectado del servidor.") cliente.close() break
Explicacion:
...
Función escribir()
def escribir(): while True: try: mensaje = input() cliente.send(mensaje.encode('utf-8'))
Explicacion:
...
hilo_recibir = threading.Thread(target=recibir) hilo_recibir.start() hilo_escribir = threading.Thread(target=escribir) hilo_escribir.start()
Explicacion:
